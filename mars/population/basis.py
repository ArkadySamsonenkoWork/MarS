...
class BasePopulator(nn.Module, ABC):
    def __init__(self,
                 context: tp.Optional[torch.Tensor] = None,
                 init_temperature: tp.Union[float, torch.Tensor] = 300.0,
                 device: torch.device = torch.device("cpu"),
                 dtype: torch.dtype = torch.float64):
        self.context = context
        self.init_temperature = self.register_buffer(
...
class BaseTimeDependantPopulator(nn.Module, ABC):
    ...
    def _init_tr_matrix_generator(self,
                                  ...
                                  *args, **kwargs) -> tr_utils.BaseMatrixGenerator:
        """
